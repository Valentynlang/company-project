<template>
  <main>
    <template v-for="section in sections">
      ...
      <DynamicTitleText v-if="section.acf_fc_layout === 'titletext'" :text="section.titletext" :paddings="section.padding_settings" :bg="section?.background_color" colspan="3" :size="section.size" />
      <DynamicTextImage v-if="['txt_img_triple_col_bg_apart', 'txt_img_sngl_withpromo_apart', 'txt_img_dbl_promo_apart', 'txt_img_dbl_kitchens_apart', 'txt_img_triple_col_bg_apart_2', 'txt_img_dbl_apart', 'txt_img_triple_no_text'].includes(section.acf_fc_layout)" :data="section.text_image_double || section.text_image_single || section.text_image_triple" :type="section.acf_fc_layout" :paddings="section.padding_settings" />
      ...
      <ApartmentsFeaturedPromo v-if="section.acf_fc_layout === 'featured_homes_promo'" :data="section" :paddings="section.padding_settings" />
    </template>
  </main>
...
<script setup>
  ...
  const { data } = await useAsyncData(
    'apartments',
    () => $fetch('https://admin.thebrentfordproject.com/wp-json/wp/v2/pages/?slug=apartments'),
  )
