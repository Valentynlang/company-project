<template>
  <main>
    <template v-for="section in sections">
      ...
      <LocationImageLabels v-if="section.acf_fc_layout === 'image_with_labels_location'" />
      <LocationGallerySlider v-if="section.acf_fc_layout === 'travel_times_slider'" :data="section.travel_time_block" :paddings="section.padding_settings" />
      ...
      <DynamicTextImage v-if="['txt_img_dbl_location', 'txt_img_sngl_withpromo_location'].includes(section.acf_fc_layout)"  :data="section.text_image_double || section.text_image_single" :type="section.acf_fc_layout" :paddings="section.padding_settings" />
      <LocationTravelTimes v-if="section.acf_fc_layout === 'travel_times'" />
      ...
      <DynamicFullScreenVideoAnimated v-if="section.acf_fc_layout === 'fullimg_vid_animated'" :video="section.fullimg_vid" :paddings="section.padding_settings" />
      <LocationLogoSection  v-if="section.acf_fc_layout === 'company_logos'" :logos="section.logos" :strapline="section.strapline" />
      <DynamicGalleryPeaks v-if="section.acf_fc_layout === 'gallery'" :gallery="section.gallery.images" :paddings="section.padding_settings" floating="bottom" />
      <AppSiteplan v-if="section.acf_fc_layout === 'siteplan'" :siteplan="section.siteplan_image" />
    </template>
  </main>
...
<script setup>
  ...
  const { data } = await useAsyncData(
    'location',
    () => $fetch('https://admin.thebrentfordproject.com/wp-json/wp/v2/pages/?slug=location'),
  )
